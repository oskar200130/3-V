...
public:
	...
	void frontera(List<T>& acu) const {
		if (_ra == NULL) return;
		else {
			fronteraAux(_ra, acu);
		}
	...
	T menorElem(T& acu) const {
		acu = _ra->_elem;
		menorElemAux(_ra, acu);
		return acu;
	}
	/**
	...
	*/
	...
	bool esAVL() const {
		int iz = 0;
		int dr = 0;
		if (_ra == NULL || esVALaux(_ra, iz, dr) != -1)
			return true;
		else
			return false;
	}
...
protected:
	...
	static int esVALaux(Nodo* aux, int& iz, int& dr) {
		if (aux == NULL)
			return 0;

		int talladr = esVALaux(aux->_dr, iz, dr);
		if (dr == 0 || dr > aux->_elem)
			dr = aux->_elem;
		else
			return -1;

		iz = -1;
		int tallaiz = esVALaux(aux->_iz, iz, dr);
		if (iz == 0 || iz < aux->_elem)
			iz = aux->_elem;
		else
			return -1;

		if (talladr == -1 || tallaiz == -1) return -1;

		if (tallaiz == talladr || tallaiz - talladr == 1)
			return 1 + tallaiz;
		else if (talladr - tallaiz == 1)
			return 1 + talladr;
		else
			return -1;
	}
	...
	void fronteraAux(Nodo* ra, List<T>& acu) const {
		if (ra->_iz == NULL && ra->_dr == NULL) acu.push_back(ra->_elem);
		else {
			if (ra->_iz != NULL)fronteraAux(ra->_iz, acu);
			if (ra->_dr != NULL)fronteraAux(ra->_dr, acu);
		}
	}

	void menorElemAux(Nodo* ra, T& acu) const {
		if (ra->_elem < acu) acu = ra->_elem;
		if (ra->_iz != NULL)menorElemAux(ra->_iz, acu);
		if (ra->_dr != NULL)menorElemAux(ra->_dr, acu);
	}
